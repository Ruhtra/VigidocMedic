...
export async function POST(req: Request) {
  try {
    ...
    console.log("[POST /api/novo/users/signup] Request Body:", JSON.stringify(body, null, 2));

    const validationResult = signupRequestSchema.safeParse(body);

    if (!validationResult.success) {
      console.error("[POST /api/novo/users/signup] Validation Error Details:", JSON.stringify(validationResult.error.format(), null, 2));
    ...
    try {
      ...
      console.error("[POST /api/novo/users/signup] Better-Auth Error:", authError);
    ...
    console.error(`[POST /api/novo/users/signup] Error:`, error);
    return NextResponse.json(
